...
function handleDegreeChange(selectorId, selectElement, numberSelectElement) {
    ...
    if (selectedNumber) {
        fetch(`/get_polynomials_view/?degree=${selectedNumber}`)
            ...
            .then(data => {
                populateSelectOptions(data, selectorId, 'id', ['first_number', 'second_number', 'letter']);
...
function handlePolynomialOperations(elementSelectId1, elementSelectId2) {
    ...
    if (selectedPolynomialId1 && selectedPolynomialId2) {
        fetch(`/handle_matrix_operations_msr_view/?polynomial_idFirst=${selectedPolynomialId1}&polynomial_idSecond=${selectedPolynomialId2}`)// передача параметрів на сервер
            .then(response => response.json())
            .then(data => {
                console.log(data);
                const matrixContainerFirst = document.getElementById('matrix-container-first');
                matrixContainerFirst.innerHTML = '';
                const matrixContainerSecond = document.getElementById('matrix-container-second');
                ...
                const polyContainerFirst = document.getElementById('poly-container-first');
                polyContainerFirst.innerHTML = '';
                const polyContainerSecond = document.getElementById('poly-container-second');
                polyContainerSecond.innerHTML = '';

                const matrixTableFirst = generateMatrixHTML(data.listResult['resultFirst'].structure_matrix);
                matrixContainerFirst.appendChild(matrixTableFirst);
                const matrixTableSecond = generateMatrixHTML(data.listResult['resultSecond'].structure_matrix);
                matrixContainerSecond.appendChild(matrixTableSecond);

                TabContainer.append('T(A) ' + data.listResult['resultSecond'].T + '; T(B) ' + data.listResult['resultSecond'].T);
                HgContainer.append('Вага(очікуване) ' + data.listResult['hg_e'] + '; Вага(реальне) ' + data.listResult['hg_r']);
                TContainer.append('T(очікуване) ' + data.listResult['T_e'] + '; T(реальне) ' + data.listResult['T_r']);

                appendListToContainer(data.listResult['sequence'], sequenceContainer);
                appendListToContainer(data.listResult['binary_sequence'], binarySequenceContainer);
                ...
                if (!nextMatrixButton) { // додавання кнопки лише якщо вона ще не існує
                    ...
                    nextMatrixButton.addEventListener('click', () => {
                        currentMatrixIndex = (currentMatrixIndex + 1) % data.listResult['matrixS'].length; // збільшуємо індекс, перевіряємо, чи не перевищили довжину масиву
                        matrixContainerS.innerHTML = '';
